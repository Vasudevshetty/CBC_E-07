  ...
  return (
    <div className="flex h-full min-h-screen">
          ...
          {sessions.map((session) => (
            ...
            >
              <div className="flex items-center overflow-hidden">
                {/* Subject color indicator - improved style */}
                <div className="flex-shrink-0">
                  <div
                    className={`w-3 h-3 rounded-full ${
                      subjectColorMap[session.subject] || "bg-gray-400"
                    } shadow-sm shadow-[#B200FF]/20 ring-1 ring-white/10`}
                  ></div>
                </div>
      ...
      <div className="flex-1 flex flex-col h-full min-h-screen bg-black bg-opacity-20">
        ...
        <div className="sticky top-0 z-10 bg-black bg-opacity-60 p-4 border-b border-[#B200FF]/20 backdrop-blur-sm">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center">
              ...
              <select
                ...
                className="bg-black bg-opacity-70 text-white border border-[#B200FF]/50 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#B200FF] shadow-sm shadow-[#B200FF]/20"
              >
        ...
        >
          {chatHistory.map((msg, index) => (
            ...
            >
              {msg.role === "assistant" && (
                <div className="h-8 w-8 rounded-full bg-black bg-opacity-70 border border-[#B200FF]/30 flex items-center justify-center mr-2 mt-1">
                  <span className="text-[#B200FF] text-sm font-semibold">
                    AI
                  </span>
                </div>
              )}
              <div
                className={`max-w-[70%] rounded-lg px-4 py-3 ${
                  msg.role === "user"
                    ? "bg-[#B200FF] bg-opacity-80 text-white shadow-lg shadow-[#B200FF]/20"
                    : "bg-black bg-opacity-70 text-white border border-[#B200FF]/20"
                }`}
                style={
                  msg.role === "user"
                    ? {
                        boxShadow: "0 0 10px rgba(178, 0, 255, 0.3)",
                        position: "relative",
                      }
                    : {}
                }
              ...
              {msg.role === "user" && (
                <div className="h-8 w-8 rounded-full bg-[#B200FF] bg-opacity-80 flex items-center justify-center ml-2 mt-1">
          ...
          {isLoading && (
            <div className="flex justify-start mb-4">
              <div className="h-8 w-8 rounded-full bg-black bg-opacity-70 border border-[#B200FF]/30 flex items-center justify-center mr-2 mt-1">
                <span className="text-[#B200FF] text-sm font-semibold">AI</span>
              </div>
              <div className="max-w-[70%] rounded-lg px-4 py-3 bg-black bg-opacity-70 border border-[#B200FF]/20">
                <div className="flex items-center">
                  <div className="flex mr-3">
                    <div className="w-2 h-2 bg-[#B200FF]/60 rounded-full animate-pulse mr-1"></div>
                    <div
                      className="w-2 h-2 bg-[#B200FF]/60 rounded-full animate-pulse mr-1"
                      style={{ animationDelay: "0.2s" }}
                    ></div>
                    <div
                      className="w-2 h-2 bg-[#B200FF]/60 rounded-full animate-pulse"
                      style={{ animationDelay: "0.4s" }}
                    ></div>
                  </div>
                  <span className="text-silver animate-glow font-medium tracking-wide">
                    {loadingText}
                  </span>
                </div>
              </div>
        ...
        <div className="sticky bottom-0 z-10 bg-black bg-opacity-60 backdrop-blur-sm border-t border-[#B200FF]/20">
          {/* Suggestions */}
            ...
            <div className="flex overflow-x-auto space-x-2 pb-3 scrollbar-thin scrollbar-thumb-[#B200FF]/50 scrollbar-track-transparent">
              {suggestions.map((suggestion, index) => (
                <button
                  ...
                  className="px-3 py-1.5 text-sm bg-black bg-opacity-70 text-gray-300 rounded-md border border-[#B200FF]/30 hover:bg-opacity-90 hover:border-[#B200FF]/70 hover:text-white hover:shadow-md hover:shadow-[#B200FF]/30 transition-all whitespace-nowrap"
                >
                  {suggestion}
                </button>
              ))}
